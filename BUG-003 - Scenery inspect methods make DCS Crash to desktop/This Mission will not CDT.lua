...
function EventHandler:onEvent( EventData )
  ...
  if EventData and EventData.initiator then
    ...
    if EventData and EventData.initiator then
      ...
      -- This won't CDT DCS world.
      -- We check if the initiator exists.
      -- When the S_EVENT_DEAD event happens, and the initiator is a SceneryObject, that is dead,
      -- the getTypeName and getCategory won't be executed.

      local IniExist = EventData.initiator and EventData.initiator:isExist()
      ...
      if TgtExist then
        ...
        Log = OLS( { TgrTypeName = TgtTypeName } )
        env.info( Log )
        trigger.action.outText( Log, 15 ) 
